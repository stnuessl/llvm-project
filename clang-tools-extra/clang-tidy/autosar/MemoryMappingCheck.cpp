...
private:
  ...
  const Keyword *PrevKeyword = nullptr;
  const Inclusion *PrevStartInc = nullptr;
  const FunctionDecl *ActiveFunctionDecl = nullptr;
  State ActiveState = State::READY;
};
...
MemoryMappingCheck::MemoryMappingCheck(StringRef Name,
    ...
    : ClangTidyCheck(Name, Context), PrevFunctionDecl(nullptr), SrcM(nullptr),
...
void MemoryMappingCheck::registerPPCallbacks(const SourceManager &SM,
  ...
  // We need to parse the preamble or otherwise we will get false postives due
  // to missing information which is contained within the preamble. We use
  // a very simple frontend action which is stripped down to only parse
  // a couple of lines of code in the main file  which empirically seems to be
  // fast enough for use with clangd.
  ...
  protected:
    bool BeginSourceFileAction(CompilerInstance &CI) override {
      // Frontend actions seem to initialize the prepocessor after calling
      // this function. This allows us to modify the preprocessor options
      // contained within the compiler instance so that the preamble
      // (and only the preamble) is getting parsed with this action.
