...
static constexpr std::pair<StringRef, StringRef> AutosarTypes[] = {
    ...
    { "StatusType", "E_OS_CALLEVEL" },
    { "StatusType", "E_OS_CORE" },
    { "StatusType", "E_OS_DISABLEDINT" },
    { "StatusType", "E_OS_ID" },
    { "StatusType", "E_OS_ILLEGAL_ADDRESS" },
    { "StatusType", "E_OS_INTERFERENCE_DEADLOCK" },
    { "StatusType", "E_OS_LIMIT" },
    { "StatusType", "E_OS_MISSINGEND" },
    { "StatusType", "E_OS_NESTING_DEADLOCK" },
    { "StatusType", "E_OS_NOFUNC" },
    { "StatusType", "E_OS_PARAM_POINTER" },
    { "StatusType", "E_OS_PROTECTION_ARRIVAL" },
    { "StatusType", "E_OS_PROTECTION_EXCEPTION" },
    { "StatusType", "E_OS_PROTECTION_LOCKED" },
    { "StatusType", "E_OS_PROTECTION_MEMORY" },
    { "StatusType", "E_OS_PROTECTION_TIME" },
    { "StatusType", "E_OS_RESOURCE" },
    { "StatusType", "E_OS_SERVICEID" },
    { "StatusType", "E_OS_SPINLOCK" },
    { "StatusType", "E_OS_STACKFAULT" },
    { "StatusType", "E_OS_STATE" },
    { "StatusType", "E_OS_VALUE" },
...
TypeUsageCheck::TypeUsageCheck(StringRef Name, ClangTidyContext *Context)
  ...
  auto RemoveOptionDispatcher = [this, &SmallTypeSet](StringRef TypeName) {
    ...
    if (!Ok && It == SmallTypeSet.end()) {
      configurationDiag("internal: unable to ignore '%0'", DiagnosticIDs::Error)
          << TypeName;
    }
  };
...
void TypeUsageCheck::dispatchCallExpr(const CallExpr *Expr) {
  ...
  if (const auto *FD = Expr->getDirectCallee()) {
    if (const auto *MD = dyn_cast<CXXMethodDecl>(FD)) {
      if (MD->isImplicitObjectMemberFunction()) {
        // Skip the implicit '*this' argument in C++ methods
        ++ExprIt;
      }
    }
  }
  ...
  // Variadic functions are always passed at least as many arguments as
  // there are parameters declared in their function prototype.
  if (TypeIt != TypeEnd || (ExprIt != ExprEnd && !FnProtoType->isVariadic())) {
...
TypeUsageCheck::TypeProxy TypeUsageCheck::visitExpr(const Expr *Expr) {
  ...
  // whether an expression is the result of a known macro expansion.
...
TypeUsageCheck::visitUnaryOperator(const UnaryOperator *UnaryOp) {
  ...
  case UO_Not:
    ...
    diag(UnaryOp->getOperatorLoc(), "invalid use of type '%0' in bit operation")
        << TypeSubExpr.getName();
    break;
  case UO_LNot:
