...
private:
  ...
  public:
    ...
    bool isSubstTemplateTypeParmType() const {
      return !Type.isNull() &&
             Type->getAs<SubstTemplateTypeParmType>() != nullptr;
    }
  ...
  TypeProxy getTypeProxy(QualType Type) {
    if (Type.isNull())
      return TypeProxy("(null)", Type);

    // Unpack C++ reference types so that they get treated by this check like
    // any other type which is sufficient for this check to work correctly.
  ...
  void dispatchEachConfigTypeValue(BinaryFunction Dispatcher) {
    auto TypeDispatcher = [this, Dispatcher](StringRef TypeName) {
      auto ValueDispatcher = [Dispatcher, TypeName](StringRef TypeValue) {
  ...
  bool checkCompatibility(const TypeProxy &LHS, const TypeProxy &RHS) {
    ...
    // Substituted template types are always compatible as they take over the
    // type of the value assigned to them.
    if (LHS.isSubstTemplateTypeParmType())
      return true;
