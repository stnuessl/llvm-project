...
TEST(MemoryMappingCheckTest, CheckOptionsInitPolicy) {
  ...
  Options.CheckOptions["autosar-memory-mapping.PolicyCleared"] =
      "BB;CCC;FFFFFF";
  Options.CheckOptions["autosar-memory-mapping.PolicyInit"] = "A;DDDD;EEEEE";
  ...
  bool runTest(std::vector<ClangTidyError> *Diagnostics, bool ApplyAdjustments,
    ...
    protected:
      bool BeginSourceFileAction(CompilerInstance &CI) override {
        ...
        // Despite being run in its own frontend action, simulate running in a
        // real clang-tidy check. The preamble is part of the main file and
        // the diagnostic engine needs to know about it so potential diagnostics
        // in the preamble can be correctly emitted.
        Context->setCurrentFile(MainFile);
        DiagEngine->setSourceManager(&SM);

        if (UsePreamble) {
          // Prepare the preprocessor options accordingly so they will be taken
          // over when the compiler instance initializes the preprocessor.
      ...
      void ExecuteAction() override {
        ...
        // The check will parse the preamble within this function.
        Check->registerPPCallbacks(SM, &PP, &PP);

        // Let the check generate its diagnostic messages.
    ...
    auto Options = std::make_unique<DiagnosticOptions>();
    ClangTidyDiagnosticConsumer DiagConsumer(Context);
    DiagnosticsEngine DiagEngine(new clang::DiagnosticIDs(), *Options,
                                 &DiagConsumer, false);

    Context.setDiagnosticsEngine(std::move(Options), &DiagEngine);
